...
class UsuarioController extends Controller
{
    ...
    public function create(): void
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $this->validateCsrf('usuarios.create');
            $dados = $this->dadosFormulario();
            $erros = $this->validarDados($dados, false);
    ...
    public function edit(): void
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $this->validateCsrf('usuarios.edit&id=' . $id);
            $dados = $this->dadosFormulario();
            ...
            $this->usuarios->update($id, $dados, $podeEditarPerfil);
    ...
    private function validarDados(array $dados, bool $isEdit): array
    {
        $erros = [];
        ...
        if (!$isEdit && strlen($dados['senha']) < 6) {
            $erros[] = 'A senha deve ter pelo menos 6 caracteres.';
        }

        if ($isEdit && $dados['senha'] !== '' && strlen($dados['senha']) < 6) {
            $erros[] = 'A nova senha deve ter pelo menos 6 caracteres.';
        }

        if (!$this->tipos->findById((int) $dados['id_tipo_usuario'])) {
            $erros[] = 'Perfil de usuário inválido.';
        }
